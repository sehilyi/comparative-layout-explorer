...
export function correctConsistency(A: Spec, B: Spec, C: CompSpec): _ConsistencySolid {
  // fill empty specs
  if (!C.consistency) {
    C = {...C, consistency: {...DEFAULT_CONSISTENCY}};
  }
  ...
  // warnings
  if (cons.y_axis != C.consistency.y_axis) console.log('consistency.y has been changed to ' + cons.y_axis)
  if (cons.x_axis != C.consistency.x_axis) console.log('consistency.x has been changed to ' + cons.x_axis)
