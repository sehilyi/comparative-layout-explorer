...
import {CompSpec, _CompSpecSolid, LayoutType, DEFAULT_COMP_SPECS, CompUnit, CompArrangement, ConsistencyType} from 'src/models/comp-spec';
...
export class AppRootBase extends React.PureComponent<AppRootProps, AppRootStates> {
          ...
          <div className="navbar navbar-dark bg-dark shadow-sm sticky-top">
            ...
            <div className="container">
              <a className="navbar-brand" href="#" onClick={() => {this.setState({view: "overview"});}}>
                ...
                <span className="navbar-brand mb-0 h1">Comparative Layout Visual Explorer</span>
              </a>
  ...
  renderGallery(specs: {A: Spec, B: Spec, C: CompSpec}) {
    ...
    return (
      <div className="card bg-light mb-3" key={key}>
        <svg ref={onBarChartC} className="img-thumbnail"></svg>
        <div className="card-body">
          <h5 className="card-title">{
            this.getLayoutName(specs.A, specs.B, specs.C)
          }</h5>
  ...
  getLayoutName(A: Spec, B: Spec, C: CompSpec) {
    ...
    if (layout.type === "juxtaposition" && layout.arrangement === "animated") return "animated transition";
    if (layout.type === "superimposition") return "superposition";

    let J = "";
    if (layout.type === "juxtaposition") {
      J += layout.unit === "chart" ? "chart-wise juxtaposition" : "item-wise juxtaposition";
      if (layout.arrangement !== "null")
        J += " (" + layout.arrangement;
      if (layout.mirrored)
        J += ", mirrored";
      J += ")"
    }
  ...
  renderEditor(specs: {A: Spec, B: Spec, C: _CompSpecSolid}) {
    ...
    return (
      <div key={key} className="container px-5 py-5">
          ...
          <div className="py-3 px-4 col-md-auto bg-light rounded-lg ">
            <div>
              ...
              <form className="form-inline">
                ...
                <select className="form-control form-control-sm col-sm-6" data-id={"difference_mark"} disabled={(specs.C.layout.type === "explicit-encoding")} value={specs.C.explicit_encoding.difference_mark ? "true" : "false"} onChange={this.handleChange.bind(this)}>
                  <option>none</option>
                  <option>true</option>
                  <option>false</option>
                </select>
